...
INSTRUCTION_OVERRIDE_PATTERNS = [
    ("ignore_previous", r"ignore\s+(all\s+)?previous\s+instructions", "critical",
     "Attempts to override previous instructions"),
    ("disregard_above", r"disregard\s+(everything\s+)?(above|previous)", "critical",
     "Attempts to disregard prior context"),
    ("forget_everything", r"forget\s+everything", "critical",
     "Attempts to clear instruction memory"),
    ("new_instructions", r"new\s+instructions\s*:", "critical",
     "Injects new instructions"),
    ("system_prompt_override", r"system\s+prompt\s+override", "critical",
     "Attempts to override system prompt"),
]

ROLE_MANIPULATION_PATTERNS = [
    ("you_are_now", r"you\s+are\s+now\b", "high",
     "Attempts to redefine the assistant's role"),
    ("pretend_you_are", r"pretend\s+(that\s+)?you\s+are", "high",
     "Attempts role manipulation via pretending"),
    ("act_as_if_instructions", r"act\s+as\s+if\s+your\s+instructions", "high",
     "Attempts to manipulate instruction interpretation"),
]

DATA_EXFILTRATION_PATTERNS = [
    ("repeat_system_prompt", r"repeat\s+your\s+system\s+prompt", "critical",
     "Attempts to extract system prompt"),
    ("output_instructions", r"output\s+your\s+instructions", "critical",
     "Attempts to extract instructions"),
    ("what_were_you_told", r"what\s+were\s+you\s+told", "high",
     "Attempts to extract instructions"),
]
        ...
        for name, pattern, severity, desc in INSTRUCTION_OVERRIDE_PATTERNS:
            ...
            if match:
                findings.append(Finding(
                    pattern_name=name, matched_text=match.group(),
                    location=location, severity=severity, description=desc,
                ))
    ...
    def _check_encoding_tricks(self, text: str, location: str) -> list[Finding]:
        ...
        if zero_width:
            findings.append(Finding(
                ...
                location=location, severity="medium",
            ...
            try:
                ...
                if any(kw in decoded for kw in suspicious_keywords):
                    findings.append(Finding(
                        ...
                        description=f"Base64-encoded suspicious content detected",
                    ))
            except Exception:
                pass
