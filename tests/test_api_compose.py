...
class TestComposeAPI:
    def _seed(self, client, sample_content):
        client.post("/api/v1/prompts", json={"slug": "reviewer", "name": "Reviewer", "type": "persona"})
        client.post("/api/v1/prompts/reviewer/versions", json={
            "content": sample_content, "message": "init",
        })
        client.post("/api/v1/prompts", json={"slug": "python-skill", "name": "Python", "type": "skill"})
        client.post("/api/v1/prompts/python-skill/versions", json={
    ...
    def test_compose(self, client, sample_content):
        ...
        resp = client.post("/api/v1/compose", json={
            "persona": "reviewer", "skills": ["python-skill"],
        })
